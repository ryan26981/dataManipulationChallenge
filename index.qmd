...
*The dtype of the actualShipDate series is datetime64[us]. I found this out by running print(shipments_df["actualShipDate"].dtype) in python. It is important that actualShipDate and plannedShipDate have the same data type for comparison because otherwise, the ‘>’ operator would not be valid (we cannot compare two variables of different types with ‘>’).
*
:::

::: {.callout-tip}
## Your Answer: String vs Date Comparison
*The result of that would be false because it would be comparaing the alphabetical value of the dates. We obviously don’t care about that. When we compare dates we compare them chronologically.
Replace this with your answer to Question 2.*
:::

::: {.callout-tip}
## Your Answer: Debug This Code
*Shipments_df[‘is_’late’] is a series of booleans, not a single boolean. I would fix it by making the if statement run row by row such as the following: 
import numpy as np
...
*The difference is how its evaluated. The first one is evaluated as a string and panda does some fancy evaluating to make sense of the expression and the other is just normal python logic. The  main difference is readability. .query(‘is_late == True’) is more readable since it essentially reads like an english sentence. However, the boolean indexing is probably more robust.
*
:::
...
late_shipments = shipments_with_lateness[
...
*The difference between drop and filter is you would use drop when you have many useful columsn and just want to remove a few useless columns, whereas you  might use filter if you have lots of columns and only want to use a select few.
*
:::

::: {.callout-tip}
## Your Answer: Handling Missing Data
*This will remove any row that has a missing value in any column, even if the fields you care about are valid. You might want to be selective about which columns to check for missing values becaues you might lose a lot more data than intended.
*
:::
...
*Ascending = false means you are going to be sorting the data largest to smallest and true means smallest to largest. You would sort multiple columns first by is late then by days late.
sorted_df = clean_shipments.sort_values(
...
*We use this to reset the row indices so that the new row indices represent the order of the sorted data. The original index when you sort remains unchanged which could be problematic since iteration could become really confusing since the numbering still reflects the old order.
*
:::
...
*Replace this with your answer to Question 1.Sum() works because booleans behave like numbers (1s and 0s). This means that when you add a bunch of true and falses together, you count the number of “trues”. In this case, the number of late rows
...
*Because we typically want to preserve our main dataset and add information from the lookup table. We can check if any shipments were lost in the merge by checking if there are any null rows in the right columns values.
*
:::

::: {.callout-tip}
## Your Answer: Key Column Matching
*The merge will be one-to-many and each shipment row for that part will get duplicated*
:::
...
*Groupby splits the data according to prodCategory. We need agg after the grouping becuase the groups are not meaningful to print out. Youd get a groupby object which isnt helpful. By using agg, you get a meaningful metric to go based off of.
*
:::

::: {.callout-tip}
## Your Answer: Multi-Level Grouping
*If we do not include shippingID, the information we get is based off of individual items. This may be helpful if for some reason, certain items tend to be particularly late. However, grouping by shippingId means that we can look at ‘for each shipment, did this category in the order have any late items
*
:::
...
This code converts the data from a line-item level to a shipment-experience level. It answers the question:

How often do customers place orders that include more than one product category, and when they do, does any category in the order arrive late?

Instead of asking whether individual parts were late, the analysis focuses on the customer’s perspective:

What does a customer actually experience when an order contains multiple types of products?*
:::
